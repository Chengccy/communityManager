...
public class UserDaoImpl  implements UserDao {
    ...
    public List<UserPOVO> getUser(String name, Integer departmentId, Integer companyId) {
        ...
        if (departmentId == -1) {
            ...
            sql = "select a.*,b.company_name,c.department_name,b.company_id " +
                    "from  user_t a, company_t b,department_t c where a.department_id=c.department_id and c.company_id=b.company_id and a.department_id=" + departmentId + " and b.company_id=" + companyId + " and a.name like '%" + name + "%'";
        }
        try {
            preparedStatement = conn.prepareStatement(sql);
            resultSet = preparedStatement.executeQuery();
    ...
    public int addUser(User user,String username) {
        ...
        try {
            ...
            preparedStatement=conn.prepareStatement("insert into operationinfo_t(table_name ,type ,content_before,content_after,operator,result) values (?,?,?,?,?,?)");
    ...
    public List<OperationInfo> getOperationInfo(String tableName, String type, String operator, String start, String end) {
        ...
        try {
            ...
            while (resultSet.next()){
                ...
                operationInfo.setType(resultSet.getString(3));
                operationInfo.setContentBefore(resultSet.getString(4));
                operationInfo.setContentAfter(resultSet.getString(5));
                operationInfo.setTime(resultSet.getString(6));
                operationInfo.setOperator(resultSet.getString(7));
    ...
    public String getLoginLogByUserId(int userId) {
        ...
        try {
            ...
            resultSet=preparedStatement.executeQuery();
            resultSet.next();
            oldLoginTIme=resultSet.getString(2);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }finally {
            if(conn!=null){
